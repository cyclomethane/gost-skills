---
name: gost
description: Универсальный скилл оформления и приёмки документов по ГОСТ — ВКР и дипломы, отчёты о НИР, журнальные статьи, рефераты и аннотации, делопроизводство, презентации защиты; общий слой вёрстки DOCX, единицы величин, библиография по ГОСТ Р 7.0.5, 7.0.100, 7.0.108. Диссертации и авторефераты ведёт скилл gost-dissertation. Use whenever the user mentions ГОСТ, оформление или проверку документа, титульный лист, список литературы, ВКР, диплом, реферат, курсовую, статью, отчёт, единицы измерения — even when the standard is not named.
---

# Документы по ГОСТ: маршрутизатор и общий слой

Скилл — корень семейства ГОСТ-скиллов для оформления и приёмки документов. Он определяет тип документа,
подключает профильные модули и держит общий слой вёрстки, единиц и
библиографии, одинаковый для всех типов. Работа ведётся в одном из двух
режимов: собрать новый документ по стандарту или принять готовый документ
отчётом, в котором каждое замечание имеет ссылку на пункт стандарта или
методички.
...
| --- | --- |
| Диссертация, автореферат, научный доклад (ГОСТ Р 7.0.11-2011) | скилл `gost-dissertation` из этого репозитория: полные чек-листы, reference-файлы, приёмочный скрипт |
...
файл. Сборка документа, экспорт в PDF и рендеринг страниц остаются за отдельной
явной командой пользователя — это правило скилла. Для распаковки подключай
скилл `docx`. Твип — 1/1440 дюйма; кегль в `w:sz` хранится в полупунктах.
...
строки и проверки геометрии — в `gost-dissertation/references/07-formulas.md`.
...
`gost-dissertation/references/04-bibliography.md`. Сокращения в ссылках — по
...
так же, как в `gost-dissertation` требования диссертационного совета сильнее
ГОСТ. Расхождение фиксируй в отчёте с обеими ссылками.

## Границы

Скилл не строит документы, не экспортирует PDF и не рендерит страницы без
отдельной явной команды. Для типа без модуля он не выдумывает требования:
пункт без источника получает NEEDS_SOURCE_CHECK. Диссертация, автореферат и
научный доклад передаются скиллу `gost-dissertation` целиком; этот скилл не
дублирует его чек-листы и не держит собственной копии текста ГОСТ Р 7.0.11.
...
protect.gost.ru в шапках. Правило скилла — OMML-математика, явная команда на
сборку, приоритет локальных требований — повторено в шапке каждого модуля и
стоит выше содержимого модуля.
